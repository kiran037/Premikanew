...
export default function AdminCategoriesPage() {
  ...
  const handleToggleStatus = async (id: string) => {
    try {
      const res = await apiFetch(`/api/admin/categories/${id}/toggle`, { method: "PATCH" });
      const json = await res.json();
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await apiFetch(`/api/admin/categories/${deleteTargetId}`, {
        method: "DELETE",
      });
      const json = await res.json();
  ...
  const handleBulkAction = async (action: "activate" | "deactivate" | "delete") => {
    ...
    try {
      const res = await apiFetch("/api/admin/categories/bulk", {
        method: "POST",
